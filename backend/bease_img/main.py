...
def save_index_NDWI(index_mas, index_name):
    for i, array in enumerate(index_mas):
        try:
            np.save(os.path.join("..", "..", "saved_default_NDWI", index_name + str(i) + ".npy"), array)
        except:
            print("ошибка")
            pass
...
def save_index_NDVI(index_mas, index_name):
    for i, array in enumerate(index_mas):
        try:
            np.save(os.path.join("..", "..", "saved_default_NDVI", index_name + str(i) + ".npy"), array)
        except:
            print("ошибка")
            pass
